...
export function groupByDirection(departures: Departure[]): DirectionGroup[] {
  ...
  // A terminus (Union) has every trip starting, or ending, at the stop itself.
  // Direction means nothing there, so show one list in time order rather than
  // columns headed "Milton +1 Durham College Oshawa".
  if (isTerminus(departures)) {
    const rows = [...departures].sort((a, b) => timeOf(a) - timeOf(b));
    return [{ key: 'all', label: 'All trains and buses, by time', shortLabel: 'By time', tone: 'c', departures: rows }];
  }
...
const timeOf = (r: Departure) => new Date(r.estimatedTime ?? r.scheduledTime).getTime();

function isTerminus(rows: Departure[]): boolean {
  return rows.every((r) => {
    const home = tidyPlace(r.stopName ?? '');
    return home !== '' && (tidyPlace(r.origin ?? '') === home || tidyPlace(r.destination ?? '') === home);
  });
}
